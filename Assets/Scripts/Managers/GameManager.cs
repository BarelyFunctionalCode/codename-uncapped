...
public class GameManager : MonoBehaviour
{
    ...
    public void StartClient(ulong hostSteamId)
	{
		...
		if (NetworkManager.Singleton.StartClient() && debugMode) Debug.Log($"{GetType()}: Client has joined!", this);
	}
    ...
    public async Task<bool> RefreshLobbies(int maxResults = 50)
	{
		...
		{
			Debug.Log($"{GetType()}: Error fetching lobbies", this);
    ...
    private void OnGameLobbyJoinRequested(Lobby lobby, SteamId id)
	{
        ...
        if (debugMode) 
        {
            Debug.Log($"{GetType()}: Owner: {lobby.Owner}");
            Debug.Log($"{GetType()}: Id: {id}");
            Debug.Log($"{GetType()}: IsSame: {isSame}", this);
        }

		StartClient(id);
	}

	private void OnLobbyInvite(Friend friend, Lobby lobby) { if (debugMode) Debug.Log($"{GetType()}: You got a invite from {friend.Name}", this); }
    ...
    {
		if (debugMode) Debug.Log($"{GetType()}: You have entered in lobby, clientId={NetworkManager.Singleton.LocalClientId}", this);
    ...
    private void OnLobbyCreated(Result result, Lobby lobby)
	{
		...
		if (debugMode) Debug.Log($"{GetType()}: Lobby has been created!");
	}

    private void ClientConnected(ulong clientId) { if (debugMode) Debug.Log($"{GetType()}: I'm connected, clientId={clientId}"); }

    private void ClientDisconnected(ulong clientId)
	{
		if (debugMode) Debug.Log($"{GetType()}: I'm disconnected, clientId={clientId}");
    ...
    {
        if (debugMode) Debug.Log($"{GetType()}: Player loaded, clientId={clientId}, sceneName={sceneName}, loadSceneMode={loadSceneMode}");
    }
    ...
    private void OnLevelLoaded(string sceneName, LoadSceneMode loadSceneMode, List<ulong> clientsCompleted, List<ulong> clientsTimedOut)
    {
        ...
        if (debugMode) Debug.Log($"{GetType()}: Level loaded, sceneName={sceneName}, loadSceneMode={loadSceneMode}, clientsCompleted={clientsCompleted.Count}, clientsTimedOut={clientsTimedOut.Count}");
    ...
    private void ChangedActiveScene(Scene current, Scene next)
    {
        ...
        if (debugMode) Debug.Log($"{GetType()}: Changed active scene from {current.name} to {next.name}");
